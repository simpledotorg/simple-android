package org.simple.clinic.registration.phone

import com.google.common.truth.Truth.assertThat
import org.junit.Test
import org.simple.clinic.registration.phone.PhoneNumberValidator.Result.Blank
import org.simple.clinic.registration.phone.PhoneNumberValidator.Result.LengthTooShort
import org.simple.clinic.registration.phone.PhoneNumberValidator.Result.ValidNumber
import org.simple.clinic.registration.phone.PhoneNumberValidator.Type.LANDLINE_OR_MOBILE

class MinimumLengthBasedNumberValidatorTest {

  private val validator = MinimumLengthBasedNumberValidator(minimumRequiredLength = 6)

  @Test
  fun `validate phone numbers`() {
    assertThat(validator.validate("", LANDLINE_OR_MOBILE)).isEqualTo(Blank)
    assertThat(validator.validate("1234", LANDLINE_OR_MOBILE)).isEqualTo(LengthTooShort(minimumAllowedNumberLength = 6))
    assertThat(validator.validate("1234567890", LANDLINE_OR_MOBILE)).isEqualTo(ValidNumber)
    assertThat(validator.validate("234567", LANDLINE_OR_MOBILE)).isEqualTo(ValidNumber)
  }
}
