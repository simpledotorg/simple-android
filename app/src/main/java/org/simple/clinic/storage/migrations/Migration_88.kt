...
@Suppress("ClassName")
class Migration_88 @Inject constructor() : Migration(87, 88) {

  override fun migrate(database: SupportSQLiteDatabase) {
    database.inTransaction {
      migrateBusinessId(this)
      migratePatientSearchResultsView(this)
    }
  }

  private fun migratePatientSearchResultsView(database: SupportSQLiteDatabase) {
    with(database) {
...
""")
  ...
  private fun migrateBusinessId(database: SupportSQLiteDatabase) {
    with(database) {
      execSQL(""" ALTER TABLE `BusinessId` ADD COLUMN "searchHelp" TEXT NOT NULL DEFAULT "" """)
      val cursor = database.query("SELECT uuid, identifier, identifierType FROM BusinessId")

      while (cursor.moveToNext()) {
        val uuid = cursor.string("uuid")
        val identifierValue = cursor.string("identifier")
        val identifierType = cursor.string("identifierType")

        if (uuid != null && identifierValue != null && identifierType != null) {
          val identifier = Identifier(
              value = identifierValue,
              type = IdentifierType.TypeAdapter.toEnum(identifierType)!!
          )
          val searchHelp = when (identifier.type) {
            BpPassport -> BpPassport.shortCode(identifier)
            else -> identifier.value
          }

          database.execSQL(""" UPDATE BusinessId SET searchHelp = "$searchHelp" WHERE uuid = "$uuid" """)
        }
      }
    }
  }
