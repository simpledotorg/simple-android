...
class CustomDrugEntryInit : Init<CustomDrugEntryModel, CustomDrugEntryEffect> {
  override fun init(model: CustomDrugEntryModel): First<CustomDrugEntryModel, CustomDrugEntryEffect> {
    return when (model.openAs) {
      is OpenAs.New.FromDrugList -> first(model, FetchDrug(model.openAs.drugUuid), LoadDrugFrequencyChoiceItems)
      is OpenAs.New.FromDrugName -> first(model.drugNameLoaded(model.openAs.drugName), LoadDrugFrequencyChoiceItems)
      is OpenAs.Update -> first(model, FetchPrescription(model.openAs.prescribedDrugUuid), LoadDrugFrequencyChoiceItems)
    }
  }
}
