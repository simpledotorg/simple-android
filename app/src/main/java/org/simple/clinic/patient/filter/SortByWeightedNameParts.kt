...
class SortByWeightedNameParts : Comparator<SearchInfo> {
  ...
  private fun computeScore(searchInfo: SearchInfo): Double {
    val max = max(searchInfo.nameParts.size, searchInfo.searchParts.size)
    ...
    val positionMultipliers = (1..max).map { it }.reversed()
    ...
    val defaultScores = (1..max).map { (1 / max.toDouble()) * positionMultipliers[it - 1] }
    ...
    searchInfo.distances.forEachIndexed { index, distance ->
      scores[index] *= (if (sumOfDistances == 0.0) 0.0 else distance / sumOfDistances)
    }
