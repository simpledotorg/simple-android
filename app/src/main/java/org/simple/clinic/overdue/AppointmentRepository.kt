...
class AppointmentRepository @Inject constructor(
  ...
  fun pendingOverdueAppointmentsInFacility(
    ...
    return overdueDao.pendingOverdueAppointmentsInFacility(
        facilityUuid = facilityId,
        scheduledAfter = since.minus(appointmentConfig.periodForIncludingOverdueAppointments),
        scheduledBefore = since
    )
  ...
  fun agreedToVisitOverdueAppointmentsInFacility(
      since: LocalDate,
      facilityId: UUID
  ): PagingSource<Int, OverdueAppointment> {
    return overdueDao.agreedToVisitOverdueAppointmentsInFacility(
        facilityUuid = facilityId,
        scheduledAfter = since.minus(appointmentConfig.periodForIncludingOverdueAppointments),
        scheduledBefore = since
    )
  ...
  fun removedOverdueAppointmentsInFacility(
    ...
    return overdueDao.removedOverdueAppointmentsInFacility(
        facilityUuid = facilityId,
        scheduledAfter = since.minus(appointmentConfig.periodForIncludingOverdueAppointments),
        scheduledBefore = since
    )
  ...
  fun moreThanAnYearOverduePatientInFacility(
      since: LocalDate,
      facilityId: UUID
  ): PagingSource<Int, OverdueAppointment> {
    return overdueDao.moreThanAnYearOverduePatientInFacility(
        facilityUuid = facilityId,
        scheduledBefore = since.minus(appointmentConfig.periodForIncludingOverdueAppointments)
    )
  }
