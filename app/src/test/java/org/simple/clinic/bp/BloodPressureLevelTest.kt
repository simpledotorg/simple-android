...
class BloodPressureLevelTest {
  ...
  fun `should correctly compute levels`() {
    ...
    val measurement1 = PatientMocker.bloodPressureMeasurement(systolic = random(0..89), diastolic = random(120..500))
    assertThat(measurement1.level).isEqualTo(BloodPressureLevel.EXTREMELY_HIGH)

    val measurement2 = PatientMocker.bloodPressureMeasurement(systolic = random(200..500), diastolic = random(0..59))
    assertThat(measurement2.level).isEqualTo(BloodPressureLevel.EXTREMELY_HIGH)

    val measurement3 = PatientMocker.bloodPressureMeasurement(systolic = random(90..129), diastolic = random(60..79))
