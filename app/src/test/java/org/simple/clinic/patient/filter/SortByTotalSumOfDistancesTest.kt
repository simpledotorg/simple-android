...
//  }
  ...
  @Suppress("Unused")
  private fun `params for sorting values`(): List<List<Any>> {
    fun generateTestData(distances: List<List<EditDistanceRecord>>, results: List<Int>): List<Any> {
      val originalList = distances.map {
        SearchContext(
            patient = PatientNameAndId(fullName = "Name", uuid = UUID.randomUUID()),
            nameParts = emptyList(),
            searchParts = emptyList(),
            editDistanceRecords = it
        )
      }
      val expected = results.map { originalList[it] }
    ...
    val template = EditDistanceRecord(namePart = "", searchPart = "", editDistance = 0.0)
