...
class PatientRepositoryAndroidTest {
  ...
  @Test
  fun querying_all_patients_in_facility_must_return_list_of_patients_partitioned_by_assigned_facility_and_order_by_name() {
    fun createPatientWithNameAndAssignedFacilityID(
        patientUuid: UUID,
        patientName: String,
        assignedFacilityId: UUID?
  ...
  @Test
  fun loading_all_patients_in_facility_paging_source_should_work_correctly() = runBlocking {
    fun createPatientWithNameAndAssignedFacilityID(
        patientUuid: UUID,
        patientName: String,
        assignedFacilityId: UUID?
    ...
    val loadResult = patientRepository
        .allPatientsInFacilityPagingSource(facility = currentFacility)
        .load(params) as Page<Int, PatientSearchResult>

    val searchResults = loadResult.data.map { it.fullName }

    //then
    assertThat(searchResults)
        .containsExactly("Patient 1", "Patient 4")
        .inOrder()
  }

  @Test
  fun searching_by_patient_name_should_work_correctly() = runBlocking {
    fun createPatientWithNameAndAssignedFacilityID(
        patientUuid: UUID,
        patientName: String,
        assignedFacilityId: UUID?
    ...
    val patientWithCurrentFacilityAsAssigned3 = UUID.fromString("9b927cc9-bc45-4beb-93dc-db5f0c1fee01")
    createPatientWithNameAndAssignedFacilityID(
        patientUuid = patientWithCurrentFacilityAsAssigned3,
    ...
    val (assignedFacilityResults, otherFacilityResults) = patientRepository.search(criteria = Name("Ram"), facilityId = currentFacility.uuid)
    ...
    assertThat(assignedFacilitySearchResults)
        .containsExactly("Ramesh", "Rama Rao")
        .inOrder()
