...
class InstantSearchEffectHandler @AssistedInject constructor(
    ...
    private val simplePagerFactory: SimplePagerFactory,
  ...
  fun build(): ObservableTransformer<InstantSearchEffect, InstantSearchEvent> = RxMobius
      ...
      .addConsumer(ShowPatientSearchResults::class.java, {
        uiActions.showPatientsSearchResults(it.assignedFacilityPatients, it.otherFacilityPatients, it.facility, it.searchQuery)
      }, schedulers.ui())
  ...
  private fun loadAllPatients(): ObservableTransformer<LoadAllPatients, InstantSearchEvent> {
    return ObservableTransformer { effects ->
      effects
          .observeOn(schedulers.io())
          .map { patientRepository.allPatientsInFacilityPagingSource(it.facility) }
          .switchMap { simplePagerFactory.createPager(config = instantSearchConfig.pagingConfig, source = it) }
