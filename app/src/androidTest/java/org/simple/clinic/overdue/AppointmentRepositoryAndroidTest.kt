...
class AppointmentRepositoryAndroidTest {
  ...
  fun deleted_blood_pressure_measurements_should_not_be_considered_when_fetching_overdue_appointments() {
    ...
    val overdueAppointments = PagingTestCase(
        pagingSource = {
          appointmentRepository.overdueAppointmentsInFacility(since = today, facilityId = facility.uuid)
        },
        loadSize = 10)
        .data
        .blockingFirst()
  ...
  fun deleted_blood_sugar_measurements_should_not_be_considered_when_fetching_overdue_appointments() {
    ...
    val overdueAppointments = PagingTestCase(
        pagingSource = {
          appointmentRepository.overdueAppointmentsInFacility(since = today,
  ...
  fun high_risk_patients_should_be_present_at_the_top_when_loading_overdue_appointments() {
    ...
    // when
    val appointments = PagingTestCase(
        pagingSource = {
          appointmentRepository.overdueAppointmentsInFacility(since = LocalDate.now(clock), facilityId = facility.uuid)
        },
        loadSize = 15)
        .data
        .blockingFirst()
  ...
  fun when_fetching_overdue_appointments_it_should_exclude_appointments_more_than_a_year_overdue() {
    ...
    //when
    val overduePatientUuids = PagingTestCase(
        pagingSource = {
          appointmentRepository.overdueAppointmentsInFacility(since = now, facilityId = facilityUuid)
        },
        loadSize = 10)
        .data
        .blockingFirst()
  ...
  fun deleted_patients_must_be_excluded_when_loading_overdue_appointments() {
    ...
    val overdueAppointments = PagingTestCase(
        pagingSource = {
          appointmentRepository.overdueAppointmentsInFacility(since = LocalDate.now(clock),
              facilityId = facility.uuid)
        },
        loadSize = 10)
        .data
  ...
  fun appointments_that_are_still_scheduled_after_the_schedule_date_should_be_fetched_as_overdue_appointments() {
    ...
    val overdueAppointments = PagingTestCase(
        pagingSource = {
          appointmentRepository.overdueAppointmentsInFacility(since = currentDate,
              facilityId = facility.uuid)
        },
        loadSize = 10)
        .data
  ...
  fun patient_that_are_marked_as_dead_should_not_be_present_when_loading_overdue_appointments() {
    ...
    val overdueAppointments = PagingTestCase(
        pagingSource = {
          appointmentRepository.overdueAppointmentsInFacility(since = now,
