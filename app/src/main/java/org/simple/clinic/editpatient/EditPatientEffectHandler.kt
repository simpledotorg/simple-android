...
class EditPatientEffectHandler @AssistedInject constructor(
          ...
          patientRepository
              .addIdentifierToPatient(
                  ...
                  identifier = Identifier(
                      value = savePatientEffect.ongoingEntry.bpPassports?.map { it.displayValue() }.toString(),
                      type = BpPassport
                  )
              )
              .ignoreElement()
  ...
  private fun isBpPassportAdded(it: SavePatientEffect) = it.ongoingEntry.bpPassports!!.isNotEmpty()
