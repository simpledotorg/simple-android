...
class BloodSugarSummaryViewUpdate : Update<BloodSugarSummaryViewModel, BloodSugarSummaryViewEvent, BloodSugarSummaryViewEffect> {
  ...
  override fun update(
    ...
    return when (event) {
      ...
      NewBloodSugarClicked -> dispatch(ShouldShowAlertFacilityChange)
      is ShowAlertFacilityChangeEvent -> {
        val effect = if (event.showAlertFacilityChange) OpenAlertFacilityChangeSheet(model.currentFacility!!) else OpenBloodSugarTypeSelector
        dispatch(effect)
      }
    }
  }
