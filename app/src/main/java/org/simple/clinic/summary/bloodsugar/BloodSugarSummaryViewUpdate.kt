...
class BloodSugarSummaryViewUpdate : Update<BloodSugarSummaryViewModel, BloodSugarSummaryViewEvent, BloodSugarSummaryViewEffect> {
  ...
  override fun update(
    ...
    return when (event) {
      ...
      NewBloodSugarClicked -> dispatch(OpenBloodSugarTypeSelector(model.currentFacility!!))
      ...
      is CurrentFacilityFetched -> next(model.currentFacilityFetched(event.currentFacility))
    }
  }
}
